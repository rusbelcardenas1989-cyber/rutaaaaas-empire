...
st.title("⚔️ Panel de Control - Modo Precisión Total")
st.write("Esta versión asegura capturar el 100% de las ciudades usando recortes numéricos limpios.")
...
@st.cache_resource
def load_ocr():
    # Buscamos solo números para evitar que las letras y líneas azules rompan la lectura
...
def procesar_captura_columnas(archivos_subidos):
    ...
    for archivo in archivos_subidos:
        ...
        # Recortes basados en la estructura de tu tabla
        crop_id = img[:, 0:int(ancho * 0.12)]
        crop_pob = img[:, int(ancho * 0.45):int(ancho * 0.62)]
        crop_edi = img[:, int(ancho * 0.88):ancho]
        
        # Lectura ultra rápida de solo números
        ...
        # Alineación magnética por altura Y en la pantalla
        for y_id, v_id in dict_ids.items():
            ...
            # Si detecta el ID y al menos un dato numérico, salvamos la fila completa
            if v_pob is not None or v_edi is not None:
                ciudades_extraidas[v_id] = {
                    "ID": v_id,
                    "Nombre": f"Ciudad {v_id}", # Nombre asignado automáticamente por ID
...
with col_izq:
    ...
    if st.session_state["mis_ciudades"]:
        # Mostramos una tabla limpia y ordenada por ID
